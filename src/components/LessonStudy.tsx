...
import { Check, Loader2, Pencil, Pin, PinOff, Plus, Sparkles, Trash2, X } from "lucide-react";
    ...
    <section className="panel p-5" id="quiz">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="flex items-center gap-2 text-base font-semibold text-white">
          <Sparkles className="h-4 w-4 text-amber-300" /> Check your understanding
        </h2>
        <p className="text-xs text-slate-400">
          ...
          {answered.length ? (
            <>
              {" · "}
              <span className={correct === answered.length ? "text-emerald-300" : "text-amber-200"}>
                {correct} correct
              </span>
            </>
          ) : null}
            ...
            <li key={q.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
              ...
              <div className="mt-3 grid gap-2">
                {q.options.map((opt, i) => {
                  ...
                  const base = "flex items-start gap-2.5 rounded-xl border px-3 py-2 text-left text-sm transition";
                  ...
                  return (
                    <button key={i} onClick={() => pick(q.id, i, chosen)} disabled={pending === q.id} className={`${base} ${cls} focus-ring`}>
                      <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-md border border-white/15 bg-white/5 text-[11px] font-bold">
              ...
              {isAnswered ? (
                <p className={`animate-fade-up mt-3 rounded-xl px-3 py-2 text-[13px] leading-6 ${isCorrect ? "bg-emerald-400/10 text-emerald-100" : "bg-amber-400/10 text-amber-100"}`}>
      ...
      <ol className="mt-4 space-y-4">
        {problems.map((p, i) => (
          <li key={p.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
            ...
            {open[p.id] === "hint" && p.hint ? <p className="mt-3 rounded-xl bg-amber-400/10 px-3 py-2 text-[13px] text-amber-100">{p.hint}</p> : null}
            {open[p.id] === "solution" ? (
              <pre className="mt-3 overflow-x-auto rounded-xl bg-slate-900/80 p-3 text-[12.5px] leading-6 text-slate-200">
...
export function NotesPanel({ lessonId, initial }: { lessonId: number; initial: NoteItem[] }) {
  ...
  return (
    <section className="panel p-5" id="notes">
      ...
      {notes.length === 0 ? (
        <p className="mt-4 rounded-xl border border-dashed border-white/15 px-4 py-6 text-center text-sm text-slate-500">
        ...
        <ul className="mt-4 space-y-3">
          {notes.map((note) => {
            const rollback = note;
            return (
              <li key={note.id} className={`rounded-xl border p-3 ${note.pinned ? "border-amber-400/25 bg-amber-400/[0.06]" : "border-white/10 bg-white/[0.02]"}`}>
                {editing === note.id ? (
                  ...
                  <>
                    <p className="whitespace-pre-wrap text-[13.5px] leading-6 text-slate-200">{note.body}</p>
                    <div className="mt-2 flex items-center gap-1 text-[11px] text-slate-500">
                      <button
                        onClick={() => patch(note.id, { pinned: !note.pinned }, rollback)}
                        className="focus-ring rounded-lg p-1.5 hover:bg-white/10 hover:text-amber-200"
                      ...
                      <button
                        ...
                        className="focus-ring rounded-lg p-1.5 hover:bg-white/10 hover:text-slate-200"
                        title="Edit"
                      >
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => remove(note.id)} className="focus-ring rounded-lg p-1.5 hover:bg-white/10 hover:text-rose-300" title="Delete">
