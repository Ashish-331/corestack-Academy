...
import { ArrowRight, BookOpen, CalendarCheck, CheckCircle2, Flame, NotebookPen, Sparkles, Target, Timer } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      {data.continueRows.length ? (
        <section>
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-slate-400">Continue learning</h2>
          <div className="grid gap-3 md:grid-cols-3">
            {data.continueRows.map((l) => (
              <div key={l.lessonSlug} className="panel panel-hover flex flex-col gap-3 p-4">
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-slate-500">{l.courseTitle}</p>
                  <Link href={`/courses/${l.courseSlug}/${l.lessonSlug}`} className="mt-1 block text-sm font-semibold text-white hover:underline">
                    {l.lessonTitle}
                  </Link>
                  <p className="mt-1 text-xs text-slate-500">{minutesLabel(l.minutes)} · {relativeTime(l.updatedAt)}</p>
                </div>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.35fr_1fr]">
        <div className="panel p-5">
          <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-400">Course progress</h2>
              ...
              <li key={c.slug}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Link href={`/courses/${c.slug}`} className="block truncate text-sm font-medium text-slate-100 hover:underline">
                      {c.title}
                    </Link>
        ...
        <div className="space-y-6">
          <div className="panel p-5">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-400">Quiz accuracy</h2>
          ...
          <div className="panel p-5">
                ...
                {data.pinnedNotes.map((n) => (
                  <li key={n.id} className="rounded-xl border border-white/10 bg-white/[0.02] p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="min-w-0 flex-1 truncate text-[13px] font-semibold text-slate-100">{n.title}</p>
                    </div>
                    <p className="mt-1 line-clamp-2 text-[12px] leading-5 text-slate-400">{n.body}</p>
                    {n.lessonSlug && n.courseSlug ? (
                      <Link href={`/courses/${n.courseSlug}/${n.lessonSlug}`} className="mt-1.5 inline-block text-[11px] text-indigo-300 hover:underline">
                        {n.lessonTitle}
                      </Link>
                    ) : null}
                  </li>
            ...
            <Link href="/notes" className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-300 hover:text-indigo-200">
      ...
      <section className="panel p-5">
        ...
        {data.recent.length === 0 ? (
          <p className="mt-3 text-sm text-slate-500">Nothing yet — complete a lesson to start the log.</p>
        ) : (
              ...
              <li key={r.id} className="flex items-center gap-3">
                <span
                  className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl ${
                    r.status === "completed" ? "bg-emerald-500/15 text-emerald-300" : "bg-indigo-500/15 text-indigo-300"
                  }`}
                >
                  {r.status === "completed" ? <CheckCircle2 className="h-4 w-4" /> : <Target className="h-4 w-4" />}
                </span>
                <span className="min-w-0 flex-1">
                  <Link href={`/courses/${r.courseSlug}/${r.lessonSlug}`} className="block truncate text-sm text-slate-200 hover:underline">
                    {r.lessonTitle}
                  </Link>
                ...
                <span className="shrink-0 text-[11px] text-slate-500">{relativeTime(r.updatedAt)}</span>
              </li>
            ))}
          </ol>
        )}
